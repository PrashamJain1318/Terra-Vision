...
interface SaveFoodDialogProps {
  dishName: string;
  onClose: () => void;
  onSave: (title: string, notes: string) => void;
}

export const SaveFoodDialog = ({ dishName, onClose, onSave }: SaveFoodDialogProps) => {
      ...
      <div className="w-full max-w-md p-6 rounded-3xl bg-card border border-border/50 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-extrabold text-foreground flex items-center gap-2">
            <Bookmark className="w-4 h-4 text-primary" /> Save Local Food Item
          </h3>
        ...
        {saved ? (
          <div className="py-6 text-center text-emerald-400 space-y-2">
            ...
            <p className="text-xs font-bold">Saved to your Culinary Favorites!</p>
          </div>
        ) : (
